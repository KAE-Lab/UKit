...
/** Ce que la copie ne reprend pas : la cle, les horodatages que la base pose, ce qui se recalcule. */
const NON_COPIE: readonly string[] = ['id', 'creee_le'];

/**
 * Une copie en brouillon, titre suffixe, pour refaire la meme affiche d'une semaine a l'autre sans
 * qu'elle parte en ligne avant d'etre relue.
 */
export async function dupliquer(ligne: Ligne): Promise<ResultatDAction> {
    ...
    return { texte: 'Copie créée en brouillon : c’est elle qui est ouverte.', ligne: ecrite };
}
