...
| [`supabase/functions/editeurs/regles.ts`](../supabase/functions/editeurs/regles.ts) | la fonction de l'équipe ([7-H](phase-7/7-h-console-roles.md)) : une demande de la page Équipe se lit ou se refuse en le disant — adresse, rôle, borne d'un rédacteur jamais vide ni donnée à un autre rôle, campus absent du catalogue nommé —, et le mot de passe provisoire a sa forme : seize signes sans ambiguïté, groupés par quatre, aucun signe favorisé |
...
| [`shared/navigation/liensDuFormulaire.ts`](../src/shared/navigation/liensDuFormulaire.ts) | la règle des domaines qui gardent le formulaire de retours dans la vue intégrée, et ceux qui s'ouvrent dehors |
| [`tools/retours/`](../tools/retours/) | l'importeur des retours du formulaire, module par module : le lecteur CSV sur ce que Google écrit vraiment, les deux écritures d'un même horodatage ramenées au même instant, le masquage des adresses et des numéros, la projection et sa **clé** — stable à travers une colonne ajoutée ou déplacée —, l'adresse d'export d'une feuille ; et l'export réel de la feuille, quand il est sur le poste (`exports/`, ignoré) |
...
| [`console/src/lib/`](../console/src/lib/) · [`console/src/schema/`](../console/src/schema/) · [`console/src/composants/liste/etatUrl.ts`](../console/src/composants/liste/etatUrl.ts) · [`console/src/pages/`](../console/src/pages/) | la console, depuis [7-E](phase-7/7-e-console-socle.md) : la traduction des erreurs de la base, la requête d'une liste (tri borné aux colonnes triables, filtres typés, recherche échappée, campus qui garde « tous campus », plage), l'état d'une liste dans l'URL, les schémas zod dérivés des descripteurs — dont les valeurs hors liste qui ne repartent pas —, la **cohérence des descripteurs** (chaque nom cité est un champ réel, du bon type ; trois erreurs trouvées le jour de sa naissance), les tables journalisées, le nom d'objet unique, les dimensions de compression, les compteurs des retours, le parc actif et l'état des annonces ; depuis [7-H](phase-7/7-h-console-roles.md), la **copie des droits** d'un rôle (`auth/droits.ts`), rejouée sur les cas du plan de test que la base joue elle-même, l'issue d'une écriture qui ne touche aucune ligne et la phrase d'un conflit (`lib/verrou.ts`), la borne d'un rédacteur dans le champ des campus, et la cohérence des colonnes lues, du verrou et des tables qui ne s'ouvrent pas |
