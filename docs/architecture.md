...
| [`shared/annonces/grammaire.ts`](../src/shared/annonces/grammaire.ts) · [`grammaire.test.ts`](../src/shared/annonces/grammaire.test.ts) | la grammaire de la description d'une annonce — blocs, titres iconés, puces, exergue, transition, signature, gras, marque de fin — rendue en arbre, et la section où tombe une ligne du texte ; pur, partagé avec la console qui dessine le même arbre en HTML et s'y cale en écrivant (7-F) ([campus-vie-etudiante.md](features/campus-vie-etudiante.md)) |
| [`shared/annonces/ordre.ts`](../src/shared/annonces/ordre.ts) · [`ordre.test.ts`](../src/shared/annonces/ordre.test.ts) | l'ordre des annonces à un instant : épinglées, créneau actif (jours ISO, heures de Paris par `Intl`, repli local), priorité, rotation déterministe par heure — pur, partagé avec le panneau d'ordre de la console (7-F), branché par la 6.3 ([7-I](phase-7/7-i-releve-et-vocabulaire.md)) |
...
| [`shared/theme/palettes.ts`](../src/shared/theme/palettes.ts) | les couleurs de base des deux thèmes, isolées pour la même raison : la console les lit pour son aperçu (7-F), `Theme.ts` les étale ([theme.md](theme.md#les-palettes)) |
