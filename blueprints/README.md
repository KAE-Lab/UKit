...
| les six `ukit-celcat-*` | un `User-Agent` qui nous nomme — `vars.user_agent`, posé en en-tête de chaque appel, sans numéro de version. Le serveur ne filtre pas dessus ; c'est une politesse envers un service public, et l'adresse à laquelle son administrateur peut écrire (docs/sources-externes.md) | groupes 2 → **3**, salles 2 → **3**, jour 3 → **4**, semaine 4 → **5**, annee 3 → **4**, occupation 2 → **3** | [7-C](../docs/phase-7/7-c-economie-et-socle.md) |
