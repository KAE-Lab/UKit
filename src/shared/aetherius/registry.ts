...
import { REMOTE_NAME_PREFIX, type BlueprintName, type PortailBlueprintName } from '../../../blueprints';
...
/**
 * Ce qu'un service a le droit de demander : un nom du socle, ou un nom de portail.
 *
 * Les seconds viennent du catalogue des etablissements, donc de la base, et ne peuvent pas figurer
 * dans `BLUEPRINT`. Le type litteral de `PortailBlueprintName` fait que seul un nom sous le prefixe
 * reserve passe — c'est la garde du registre, avancee jusqu'au compilateur.
 */
export type RunnableBlueprintName = BlueprintName | PortailBlueprintName;

/**
 * Ce nom vient-il du prefixe reserve ?
 *
 * Le registre rend des noms en `string` : `list()` enumere le socle **et** ce qui est arrive par la
 * porte du jalon 3-H. Ce predicat les distingue sans forcer un `as`, qui affirmerait sans verifier.
 */
export function estNomDePortail(name: string): name is PortailBlueprintName {
 ...
 *
 * Un nom de portail qui n'a pas encore ete livre leve, comme n'importe quel nom inconnu : il n'y a
 * rien a jouer, et repondre autre chose serait inventer un socle. C'est la contrepartie assumee de la
 * levee de garde — un portail ajoute a distance n'a pas de repli hors ligne avant d'avoir ete resolu
 * une fois, et c'est sans consequence : un portail qu'on n'a jamais joue n'a rien a quoi retomber.
 *
 * @throws {BlueprintError} le document n'est pas un Blueprint valide, ou ce nom n'existe pas.
 */
export async function resolveBlueprint(name: RunnableBlueprintName): Promise<ResolvedBlueprint> {
...
export interface BlueprintLine {
    readonly name: RunnableBlueprintName;
 ...
 * resolution.
 *
 * Depuis le jalon 6-G, la liste porte aussi les **portails ajoutes a distance** : ce sont eux qu'on
 * vient verifier quand une publication « n'arrive pas », et les voir passer en « distant » est la
 * preuve la plus directe que le mecanisme a fonctionne.
 */
export async function describeDelivery(): Promise<readonly BlueprintLine[]> {
    ...
    for (const status of statuses) {
        // Le registre rend des `string` : un nom vient soit du socle, soit du prefixe reserve.
        // Le predicat evite d'affirmer par un `as` ce qu'on peut verifier en une comparaison.
        const name: RunnableBlueprintName = estNomDePortail(status.name)
            ? status.name
            : (status.name as BlueprintName);
        try {
 ...
 */
export async function revertBlueprints(name?: RunnableBlueprintName): Promise<void> {
