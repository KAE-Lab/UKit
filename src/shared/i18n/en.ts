/* eslint-disable max-lines -- un dictionnaire est un fichier de donnees : le decouper nuirait a sa lecture, comme Theme.ts */
const en = {
	ABOUT: 'About',
	ADD_TO_CALENDAR: 'Add event to calendar',
	ADD_TO_CALENDAR_DESCRIPTION: 'Would you like to add the event "$-" to calendar?',
	ADD_TO_CALENDAR_DONE: 'Event added!',
	ADD_TO_CALENDAR_PERMISSIONS:
		'This application needs permissions in order to add events to calendar',
	APP_LAUNCHING: 'Start-up',
	APP_READY: 'The app is now ready, you can now check your class schedule!',
	APPLICATION: 'Application',
	APPLICATION_HISTORY:
		'This application is a fork of UKit v4.1.2, modernized and maintained by the KAE-Lab team.\n\nPowered by React Navigation 6.',
	AUTO_SYNC_DESCRIPTION:
		'Sync runs on its own about every 12 hours, when the system allows it, and when the app opens if the last one is more than 12 hours old.',
	AUTUMN: 'Autumn',
	BACHELORS: "Bachelor's",
	CANCEL: 'Cancel',
	CLOSE: 'Close',
	CALENDAR: 'Calendar',
	CALENDAR_SYNCHRONIZATION: 'Synchronization with a calendar',
	COMPANY_WEBSITE: 'Company website',
	CONFIDENTIALITY_POLITIC: 'Confidentiality politic',
	CONFIRM: 'Confirm',
	CONTACT_US: 'Contact us',
	DARK_THEME: 'Dark',
	DAY: 'Day',
	DETAILS: 'Details',
	DISPLAY: 'Display',
	ENABLE_CALENDAR_PERMISSION_DESCRIPTION:
		'Please allow Ukit to access to your calendars in system Settings to use this feature.',
	ENGLISH: 'English',
	// Le modele d'erreur du moteur : une famille d'echec, un message (shared/aetherius/failures.ts).
	ERROR_SERVICE_UNAVAILABLE_TITLE: 'Service unavailable',
	ERROR_SERVICE_UNAVAILABLE: 'Check your connection, then try again.',
	ERROR_UNEXPECTED_RESPONSE_TITLE: 'Unexpected response',
	ERROR_UNEXPECTED_RESPONSE: 'Your university website sent a response that could not be read.',
	ERROR_CONTENT_NOT_FOUND_TITLE: 'Content not found',
	ERROR_CONTENT_NOT_FOUND: 'Your university website has changed, and UKit can no longer find its way.',
	ERROR_BLOCKED_TITLE: 'Connection interrupted',
	ERROR_BLOCKED: 'The connection was interrupted before it completed.',
	ERROR_MISSING_CREDENTIALS_TITLE: 'Something is missing',
	ERROR_MISSING_CREDENTIALS: 'Something is missing to continue. Check your credentials in your account.',
	ERROR_INTERNAL_TITLE: 'Something went wrong',
	ERROR_INTERNAL: 'This operation could not be completed. The problem is on UKit’s side.',
	// Les echecs que les Blueprints de scolarite se donnent eux-memes (`fail:CODE`). Ils sont plus
	// precis que la famille dont ils relevent : « bloque » ne dirait pas lequel des deux services a
	// manque (features/Scolarite/services/ScolariteMapping.ts).
	ERROR_CAS_UNAVAILABLE_TITLE: 'Portal unreachable',
	ERROR_CAS_UNAVAILABLE: 'Your university portal is not responding. Try again in a moment.',
	ERROR_MAILBOX_UNAVAILABLE_TITLE: 'Mailbox unreachable',
	ERROR_MAILBOX_UNAVAILABLE: 'The mailbox did not respond, but your sign-in worked.',
	// Tout code de Blueprint en `_INDISPONIBLE` que la table ne nomme pas precisement : la regle
	// de ScolariteMapping le presente comme un service injoignable, reessayable (6.1-A).
	ERROR_PORTAL_SERVICE_UNAVAILABLE_TITLE: 'Service unreachable',
	ERROR_PORTAL_SERVICE_UNAVAILABLE: 'This university service is not responding. Try again in a moment.',
	// Un service que l'etablissement ne publie pas : rien n'est en panne, il n'y a rien a joindre.
	// Le distinguer d'une panne est le point du jalon 6-G (shared/etablissements/catalogue.ts).
	ERROR_PORTAL_UNAVAILABLE_TITLE: 'Campus not connected yet',
	ERROR_PORTAL_UNAVAILABLE: 'This campus is not connected to UKit yet.',
	// Le cas general de la meme famille : un service que cet etablissement ne propose pas. L'ecran le
	// fait le plus souvent **disparaitre** plutot que de l'afficher ; le message existe pour les
	// chemins ou il reste atteignable.
	ERROR_SERVICE_ABSENT_TITLE: 'Service not offered',
	ERROR_SERVICE_ABSENT: 'Your university does not offer this service in UKit.',
	ERROR_TIMETABLE_UNAVAILABLE_TITLE: 'No timetable here',
	ERROR_TIMETABLE_UNAVAILABLE: 'This university does not publish its timetable in UKit yet.',
	ERROR_TIMETABLE_GROUP_UNKNOWN_TITLE: 'Group not found',
	ERROR_TIMETABLE_GROUP_UNKNOWN: 'This group no longer exists in your university timetable. Pick another one.',
	// Distinct de ERROR_TIMETABLE_UNAVAILABLE, et c'est tout l'objet du jalon 6-J : la-bas rien
	// n'existe, ici il manque un geste. Deux phrases, deux ecrans, deux gestes attendus.
	ERROR_TIMETABLE_LINK_MISSING_TITLE: 'A link to paste',
	ERROR_TIMETABLE_LINK_MISSING: 'The timetable shows up here from a subscription link (iCal). Paste yours to display it.',
	TIMETABLE_GROUPS_MISSING: 'Groups not found: $-',
	// Une donnee presente mais incomplete : une source interrogee en plusieurs points n'a pas
	// repondu partout. Ce n'est pas un echec, c'est une reserve sur ce qui est affiche.
	PARTIAL_COVERAGE: 'This list may be incomplete: some responses are missing.',
	EXISTING_CALENDARS: 'Existing calendars:',
	FILTERS: 'Filters',
	FILTERS_CONFIRMATION:
		'Would you like to filter out this UE? You can always revert this action in the settings.',
	FILTERS_ENTER_CODE: "Enter in textbox UE codes you don't want to see.",
	FILTERS_ACTIVE: 'Active filters',
	FILTERS_ADD: 'Add a filter',
	FILTERS_UE: 'Filter out the UE',
	FILTER_MODE_EXCLUDE: 'Hide',
	FILTER_MODE_INCLUDE: 'Show only',
	FAVORITES_MANAGE: 'Favourite groups',
	FAVORITES_EMPTY_TITLE: 'Empty timetable',
	FAVORITES_EMPTY: 'Add a group to your favourites to see its timetable here.',
	SEARCH_UE: 'Search for a UE...',
	AVAILABLE_UES: 'Available UEs',
	NO_UE_FOUND: 'No UE found',
	FINISH: 'Finish',
	FORCE_SYNC: 'Force a sync',
	FRENCH: 'French',
	GROUPS: 'Groups',
	GROUP_NAME: 'Group name...',
	HIDDEN_RESULT: 'There are $- hidden results',
	LANGUAGE: 'Language',
	LAST_SYNCHRONIZATION: 'Last synchronization',
	LEGAL_NOTICE: 'Legal Notice',
	LIGHT_THEME: 'Light',
	MAILBOX: 'Mailbox',
	MASTERS: "Master's",
	MY_GROUP: 'My group',
	MY_PLANNING: 'Planning',
	NAVIGATION: 'Navigation',
	NEXT: 'Next',
	NONE: 'None',
	NO_CLASS_THIS_DAY_TITLE: 'Free day',
	NO_CLASS_THIS_DAY: 'No class on this day.',
	NO_CONNECTION: 'No connection',
	NO_FILTER: 'No filter currently',
	NO_GROUP_FOUND_WITH_THIS_SEARCH_TITLE: 'No group',
	NO_GROUP_FOUND_WITH_THIS_SEARCH: 'No group matches this search.',
	NO_SYNCHRONIZATION_DONE: 'No synchronization done',
	LAST_SYNCHRONIZATION_FAILED: 'Last synchronization failed',
	LAST_ATTEMPT_FAILED_AT: 'Last attempt failed $-',
	CALENDAR_SYNC_FAILED_TOAST: 'Synchronization failed. The calendar was left unchanged.',
	SYNC_NEEDS_CALENDAR: 'Choose a destination calendar first.',
	SYNC_NEEDS_FAVORITES: 'Add a favorite group first: it is what fills the calendar.',
	NOTIFICATIONS_REFUSEES: 'Notifications are turned off for UKit. Allow them in your phone settings.',
	NOT_FOUND: 'Not found',
	OFFLINE_DISPLAY_FROM_DATE: 'Offline display from $-',
	OPEN_SYSTEM_SETTINGS: 'Open system settings',
	// Les calendriers du telephone dans le Planning (jalon 6.1.x-D).
	PHONE_CALENDARS: 'Phone calendars',
	PHONE_CALENDARS_DESCRIPTION: 'Their events appear with your classes, in the day and week views, in the color of their calendar. Nothing is shown until you check a calendar. The calendar used for synchronization is not offered: it already holds your classes.',
	PHONE_CALENDARS_COUNT: '$- selected',
	PHONE_CALENDARS_NONE: 'None',
	PHONE_CALENDARS_EMPTY: 'This phone has no calendar.',
	ALL_DAY: 'All day',
	OPEN_IN_CALENDAR: 'Open in calendar',
	OPEN_IN_CALENDAR_FAILED: 'Couldn’t open the event in the calendar',
	GETTING_THERE_FAILED: 'Couldn’t open the map',
	ADD_EVENT: 'Add an event',
	OPEN_ON_FAVOURITE_GROUP: 'Open on favourite group',
	OTHER: 'Other',
	RESET: 'Reset',
	// Bouton d'un ecran en echec, quand la famille est reessayable (shared/aetherius/failures.ts).
	RETRY: 'Retry',
	DISABLE_SYNC: 'Turn off synchronisation',
	DISABLE_SYNC_CONFIRMATION:
		'The courses already added to your calendar will be removed. Your UKit timetable is untouched.',
	DISABLE: 'Turn off',
	RESET_APP: 'Reset application',
	RESET_APP_CONFIRMATION: 'Do you want to reset the application? You cannot undo this action.',
	SAVE: 'Save',
	SETTINGS: 'Settings',
	SETTINGS_TO_MAKE: 'A few settings before you start.',
	SEARCH: "Search",
	SPANISH: 'Spanish',
	SPRING: 'Spring',
	START: 'Start',
	SYNCHRONIZING: 'Synchronizing',
	SYNC_ENABLED: 'Auto sync. enabled',
	SYNC_NOW: 'Sync. now',
	THIS_WEEK: 'This week',
	TODAY: 'Today',
	UKIT_CALENDAR: 'New UKit calendar',
	UPDATE_UKIT: 'Update',
	UPDATE_UKIT_DESCRIPTION: 'A new version is available. Updating is recommended.',
	USE_SEARCH_BAR: 'Please use the search bar to narrow down the results.',
	YOUR_CALENDAR: 'Which calendar must be use for synchronization?',
	// Le multi-etablissement (jalon 6-G). Le NOM d'une universite vient du catalogue, jamais d'ici :
	// ce sont les libelles d'ecran qui se traduisent, pas les donnees.
	YOUR_INSTITUTION: 'Which university do you attend?',
	INSTITUTIONS_LOADING: 'Fetching the list of institutions…',
	GROUPS_LOADING: 'Fetching the list of groups…',
	// --- Les chargements qui parlent (6.1-E) : ce qu'on attend, et la lenteur quand elle dure
	LOADING_TIMETABLE: 'Your timetable is on its way…',
	LOADING_FREE_ROOMS: 'Looking for free rooms…',
	LOADING_CAMPUS_OPEN: 'Checking what is open…',
	LOADING_ANNOUNCEMENTS: 'Loading announcements…',
	LOADING_MENU: 'Loading the menu…',
	LOADING_OPENING_HOURS: 'Loading opening hours…',
	LOADING_DOCUMENT: 'Opening the document…',
	LOADING_PORTAL: 'The portal is loading…',
	LOADING_PATIENCE: 'This is taking longer than usual…',
	LOADING_PATIENCE_UNIVERSITY: 'The university server is slow today…',
	INSTITUTION: 'Institution',
	INSTITUTION_CHANGE_TITLE: 'Switch institution?',
	INSTITUTION_CHANGE_WARNING: 'Your favourite groups, cached timetable, university session and the courses added to your calendar will be erased.\nData from two universities does not mix.',
	INSTITUTION_UNAVAILABLE: 'Your institution is no longer listed. UKit carries on with what it knows.',
	// Le compte a l'accueil et le lien d'abonnement (jalon 6-J).
	UNIVERSITY_ACCOUNT: 'University account',
	ACCOUNT_CONNECTED: 'Signed in',
	ACCOUNT_NOT_CONNECTED: 'Not signed in',
	CONNECT_ACCOUNT_TITLE: 'Sign in to your university account',
	CONNECT_ACCOUNT_DESC:
		'To bring back your record, your messages and your timetable. Your credentials stay encrypted on your phone, and are only ever sent to your university.',
	LATER: 'Later',
	OTHER_CAMPUS_QUESTION: 'From another campus?',
	TIMETABLE_LINK: 'Timetable link',
	TIMETABLE_LINK_TITLE: 'Paste your timetable link',
	TIMETABLE_LINK_DESC:
		'Your university publishes its timetable through a subscription link (iCal). Paste it here: it stays on your phone, encrypted, like a password.',
	TIMETABLE_LINK_PLACEHOLDER: 'https://…',
	TIMETABLE_LINK_ADD: 'Paste my link',
	TIMETABLE_LINK_CHECK: 'Check and save',
	TIMETABLE_LINK_CHECKING: 'Checking…',
	TIMETABLE_LINK_OK: 'Calendar recognised: $- classes.',
	TIMETABLE_LINK_EMPTY: 'This calendar has no classes in it. Check that the link is really yours.',
	TIMETABLE_LINK_INVALID:
		'This link does not return a calendar. Check that you copied the subscription link (iCal), not the address of your timetable page.',
	TIMETABLE_LINK_FORGET: 'Forget this link',
	TIMETABLE_LINK_HELP: 'Where to find it:',
	YOUR_GROUP: 'What is your group?',
	YOUR_LANGUAGE: 'What is your language?',
	YOUR_SEMESTER: 'What semester are you in?',
	YOUR_THEME: 'Choose a theme for the application',
	YOUR_YEAR: 'What is your year?',
	WEBSITE: 'Website',
	WELCOME: 'Welcome!',
	WELL_DONE: 'Well done!',
	WEEK: 'Week',
	WEB_BROWSER: 'Web browser',
	GROUPS_LIST: 'Groups list',
	RESTAURANTS_U: 'Restaurants',
	RESTAURANTS: 'Restaurants',
	MENU: 'Menu',
	MESSAGE_COMPRIS: 'Got it',
	SERVICE_OK_TITLE: 'All clear',
	SERVICE_OK_BODY: 'Every service is up and the app is running as intended. (For now, at least.)',
	SERVICE_REPORT_LINK: 'Found a bug, got an idea? Tell us',
	LUNCH: 'Lunch',
	DINNER: 'Dinner',
	DAY_MON: 'Mon',
	DAY_TUE: 'Tue',
	DAY_WED: 'Wed',
	DAY_THU: 'Thu',
	DAY_FRI: 'Fri',
	DAY_SAT: 'Sat',
	DAY_SUN: 'Sun',
	BU_OPEN: "Open",
	BU_CLOSED: "Closed",
	BUILDING_CLOSED_TITLE: "Building closed",
	BUILDING_CLOSED: "Come back during opening hours to see free rooms.",
	BUILDING_CLOSED_LABEL: 'Closed',
	CAMPUS: 'Campus',
	LIBRARIES: 'Libraries',
	NO_BU_NEARBY_TITLE: 'No library',
	NO_BU_NEARBY: 'No library found around you.',
	LIBRARY_DETAILS: 'Library details',
	ADRESS: 'Address',
	ATTENDANCE: 'Live attendance',
	CLOSED_ALL_DAY: 'Closed all day',
	BOOK_SEAT: 'Book a seat',
	OPENING_HOURS: 'Opening hours',
	OCCUPANCY_RATE: 'Occupancy rate',
	COURSES: 'Courses',
	DATA_SOURCE_TITLE: 'Data source',
	DATA_SOURCE_TEXT: "Schedules are retrieved from your institution's ENT. This application is an independent project and is not officially affiliated with the institution.",
	LEGAL_MENTION: 'This project is distributed under free license. Dependencies use MIT and Apache 2.0 licenses.',
	CREDITS_API: 'Credits & API',
	THIRD_PARTY_DESC: 'This application uses third-party services to provide real-time data to students:',
	API_AFFLUENCES: '• API Affluences',
	AFFLUENCES_DESC: 'Real-time schedules and attendance for libraries (BU).',
	API_CROUSTILLANT: '• API CROUStillant',
	CROUSTILLANT_DESC: 'Daily menus and images for university restaurants provided by',
	NO_MENU_PUBLISHED_TITLE: 'No menu',
	NO_MENU_PUBLISHED: 'No menu published for this restaurant currently.',
	NOT_SPECIFIED: 'Not specified',
	NO_DISH_INFO: 'No dish information for this day.',
	RESTAURANT_U: 'University restaurant',
	UNSPECIFIED_HOURS: 'Unspecified hours',
	CATEGORY: 'Category',
	UNKNOWN: 'Unknown',
	LIBRARY: 'Library',
	TIME_SEPARATOR: ':',
	GETTING_THERE: 'Getting there',
	WEEK_SHORT: 'W-',
	CALENDAR_ERROR: "Couldn't add event to calendar",
	ROOM: 'Room',
	UNKNOWN_SUBJECT: 'Unknown course',
	SETTINGS_LOAD_ERROR: "Settings couldn't be loaded",
	NO_RU_NEARBY_TITLE: 'No restaurant',
	NO_RU_NEARBY: 'No university restaurant around you.',
	SCOLARITY: "Academics",
	ANNOUNCEMENTS: "Announcements",
	SEE_ALL: "See all",
	UNIVERSITY_LIBRARY: "Libraries",
	FREE_ROOMS: "Free Rooms",
	// Campus screen labels. They were missing from all three dictionaries until milestone 6-K: eight
	// of them were rendered as raw keys on screen, including the empty-state message.
	SEARCH_BU_CITY: 'Search for a library or a city...',
	SEARCH_RESTO_CITY: 'Search for a restaurant or a city...',
	SEARCH_BUILDING: 'Search for a building...',
	ALL_LIBRARIES: 'All libraries',
	OPEN_LIBRARIES: 'Open now',
	ALL_ESTABLISHMENTS: 'All venues',
	RESTO_U: 'Restaurants',
	CROUS_MARKET: 'Crous Market’',
	ROOMS: 'rooms',
	NO_RESULTS_TITLE: 'Nothing on',
	NO_RESULTS: 'No announcements right now.',
	NO_RESULTS_FOUND_TITLE: 'No result',
	NO_RESULTS_FOUND: 'Nothing matches your search or your filters.',
	SHOW_ALL: 'Show all',
	SECTION_ALL_FILTERED: 'Everything is hidden by your filter.',
	NO_BUILDING_FOUND_TITLE: 'No building',
	NO_BUILDING_FOUND: 'No open-access building nearby.',
	NO_FREE_ROOMS: 'No free room at this time.',
	AVAILABLE_UNTIL: "Available until",
	FLOOR_GROUND: 'Ground floor',
	FLOOR_N: 'Floor $-',
	FLOOR_UNNUMBERED: 'Other rooms',
	LOGS: "Logs",
	LOGOUT: 'Log out',
	REENTER_CREDENTIALS: 'Re-enter',
	REFRESH_RECORD: 'Refresh',
	REFRESH_RECORD_DESC: 'Connecting to the portal again will bring the record up to date. It takes a few seconds.',
	CONNECT: "Log in",
	CONNECTING: 'Signing in…',
	LOGIN_FAILED_TITLE: 'Credentials refused',
	LOGIN_FAILED: 'Check your username and password.',
	LOGIN_NETWORK_ERROR: "Unable to reach the portal. Check your connection.",
	CONFIRM_LOGOUT: "Are you sure you want to log out?",
	CONNECTED_AS: "Logged in as",
	MESSAGING: "Mailbox",
	MAILBOX_NO_UNREAD: 'All read',
	MAILBOX_UNREAD_ONE: "$- unread message",
	MAILBOX_UNREAD_MANY: "$- unread messages",
	CREDENTIALS_SETTINGS: "Credentials",
	USERNAME: "Username",
	PASSWORD: "Password",
	DELETE_CREDENTIALS: "Delete credentials",
	SAVE_CREDENTIALS: "Save credentials",
	NO_CREDENTIALS_SAVED: "No credentials saved.",
	ENTER_CREDENTIALS_DESC: "Enter your ENT credentials to log in automatically.",
	CREDENTIALS_SAVED_SUCCESS: "Credentials saved successfully!",
	CREDENTIALS_DELETED_SUCCESS: "Credentials deleted.",
	SAVE_CREDENTIALS_PROMPT: 'Remember these credentials to fill this form on its own next time? They stay encrypted on the phone and open no session in UKit.',
	BROWSER_AUTOFILL_TITLE: 'Browser credentials',
	BROWSER_AUTOFILL_DESC: 'Remembered to fill the services sign-in',
	BROWSER_AUTOFILL_FORGET: 'Forget these credentials',
	BROWSER_AUTOFILL_FORGET_DESC: 'The built-in browser will no longer fill the sign-in form. Nothing else changes.',
	CONFIRM_SAVE_CREDENTIALS: "Are you sure you want to save these credentials?",
	CONFIRM_DELETE_CREDENTIALS: "Are you sure you want to delete these credentials?",
	YES: "Yes",
	NO: "No",
	PORTAL_NO_CREDENTIALS_TITLE: "Credentials required",
	PORTAL_NO_CREDENTIALS_DESC: "Save your ENT credentials to access this feature.",
	PORTAL_ADD_CREDENTIALS: "Add my credentials",
	PORTAL_CONNECTING: 'Signing in…',
	PORTAL_TAP_TO_OPEN: "Tap to open",
	PORTAL_SECTION_TITLE: "University portals",
	GREETING_DAY: "Hello",
	GREETING_EVENING: "Good evening",
	HAPPY_BIRTHDAY: "Happy birthday",
	LOADING_CONNECTING: 'Connecting to the portal…',
	LOADING_PROFILE: 'Reading the profile…',
	LOADING_DOSSIER: 'Reading the student record…',
	BIOMETRY_TITLE: 'Protected content',
	BIOMETRY_PROMPT: 'Authenticate to see your information',
	BIOMETRY_FALLBACK: "Use device passcode",
	BIOMETRY_RETRY: "Try again",
	SHOW_PASSWORD: "Show password",
	STUDENT_NUMBER: "Student number",
	STUDENT_INE: "INE",
	FIRST_NAME: 'First name',
	DATE_OF_BIRTH: "Date of birth",
	STUDENT_EMAIL: "University email",
	PROFILE: "Profile",
	DOSSIER: "Record",
	THEME: "Theme",
	DARK_MODE: "Dark Mode",
	NOTIFICATIONS: "Notifications",
	COURSE_NOTIFICATIONS: "Course Notifications",
	NOTIFICATION_DELAY: "Notification delay",
	NOTIFICATION_DELAY_VALUE: '$- min',
	NOTIFICATION_DELAY_DESC: "Adjust how many minutes before the start of the course you want to be notified.",
	NOTIFICATION_CAP_DESC: "The next 20 classes of the week last opened are covered.",
	PUSH_MESSAGES: 'Service messages',
	PUSH_MESSAGES_DESC: 'An incident or important news reaches you even when the app is closed. To do so, the app deposits a push token with what it takes to target you: campus, version, platform. Turning this off removes it.',
	PRIVACY_SECTION: 'Privacy',
	ANONYMOUS_STATS: 'Anonymous statistics',
	ANONYMOUS_STATS_DESC: 'To learn what is useful, the app counts a few gestures: an opening, a tab, an announcement seen. Numbers per day and per campus, never an identifier nor anything you type, sent when the app goes to the background. Turning this off erases what was waiting.',
	NOTIFICATION_COURSE_IN: "Class in $- min",
	NOTIFICATION_COURSE_FALLBACK: "Class",
	NOTIFICATION_LOCATION_UNKNOWN: "Room unknown",
	PROPOSALS_TITLE: "Found in your records",
	PROPOSALS_INTRO: 'Your university record has a few things worth adding to your profile. Nothing will be applied without your consent.',
	PROPOSALS_EDT_LABEL: 'Timetable',
	PROPOSALS_EDT_HINT: "It will be added to your favourites, like a group. You can remove it at any time.",
	PROPOSALS_UES_LABEL: 'Courses not taken',
	PROPOSALS_UES_HINT: "They will be hidden from your timetable. You can show them again in the settings.",
	PROPOSALS_APPLY: "Apply",
	PROPOSALS_SKIP: "No thanks",

	// Academics tab: the three sections of the redesign, and what they carry.
	MY_DOCUMENTS: 'Documents',
	RECORD_NEVER_READ: 'Never',
	PROGRAMME: "Programme",
	SERVICE_ENT_SUBTITLE: 'University portal',
	SERVICE_MOODLE_SUBTITLE: "Learning platform",
	SERVICE_APOGEE_SUBTITLE: "Marks and results",
	SERVICE_ENT: "ENT",
	SERVICE_MOODLE: "Moodle",
	SERVICE_APOGEE: "Apogée",
	// Scolarity widgets: one row per service, and what it announces.
	SERVICE_MAILBOX: "Mailbox",
	SERVICE_MAILBOX_SUBTITLE: 'University inbox',
	SERVICE_NOTES: "Grades and results",
	SERVICE_NOTES_SUBTITLE: 'Transcripts by term',
	SERVICE_EXAMS: "Exams",
	SERVICE_EXAMS_SUBTITLE: 'Exam dates',
	WIDGET_MOODLE_NONE: "Nothing due",
	WIDGET_MOODLE_ONE: "$- assignment due",
	WIDGET_MOODLE_MANY: "$- assignments due",
	WIDGET_NOTES_NONE: "No new grade",
	WIDGET_NOTES_ONE: "$- new grade",
	WIDGET_NOTES_MANY: "$- new grades",
	WIDGET_EXAMS_NONE: 'No exam coming up',
	WIDGET_EXAMS_ONE: "$- exam coming up",
	WIDGET_EXAMS_MANY: "$- exams coming up",
	WIDGET_MAILBOX_UNIT: "unread",
	WIDGET_MAILBOX_UNIT_ONE: "unread",
	SCOLARITE_SECTION_GLANCE: "At a glance",
	SCOLARITE_SECTION_SERVICES: "Your services",
	COMING_SOON_TITLE: "Coming soon",
	COMING_SOON_BODY: "It arrives in an upcoming update.",
	COMING_SOON_OPEN: "Open the service in the meantime",
	COMING_SOON_ACK: "Got it",
	OPENFREEMAP_DESC: "Map backgrounds, based on OpenStreetMap data.",
	SEARCH_ANNONCE: "Search for an announcement...",
	WIDGET_MOODLE_UNIT: "due",
	WIDGETS_REFRESHED: "Updated $-",
	WIDGET_NOT_CARRIED: 'Not available yet',
	// Les deux mots d'une tuile en echec, et le geste de sa feuille (6.1-A).
	WIDGET_FAILURE_REENTER: 'Sign-in needed',
	WIDGET_FAILURE_UNAVAILABLE: 'Unavailable',
	WIDGET_FAILURE_ERROR: 'Error',
	WIDGET_RELAUNCH: 'Relaunch',
	CONNECT_ACCOUNT_ACTION: "Sign in",
	NO_DOCUMENTS_TITLE: "No documents",
	NO_DOCUMENTS: 'Keep your enrolment certificates and statements here. They stay on your device, and open even without a connection.',
	DOCUMENTS_UNIT: "files stored",
	DOCUMENT_UNIT_ONE: "file stored",
	DOCUMENTS_SUBTITLE: 'On this device',
	// Le nom sous lequel le certificat de scolarite est range quand l'application va le chercher
	// elle-meme. C'est un LIBELLE D'ECRAN, donc traduit : le portail, lui, nomme sa piece dans sa
	// propre langue et c'est ce nom-la qui gagne quand il existe (services/CertificatProjection.ts).
	DOCUMENT_CERTIFICATE: 'Enrolment certificate',
	ADD_DOCUMENT: "Add a document",
	DELETE_DOCUMENT: "Delete this document",
	CONFIRM_DELETE_DOCUMENT: "This document will be removed from your device. This cannot be undone.",
	DOCUMENT_ADD_FAILED: "Could not add this document.",
	DOCUMENT_OPEN_ELSEWHERE: 'Open elsewhere',
	DOCUMENT_PREVIEW_UNAVAILABLE_TITLE: 'Preview unavailable',
	DOCUMENT_PREVIEW_UNAVAILABLE: 'This kind of document cannot be shown here. It opens in another app, without leaving the device.',
	DOCUMENT_PREVIEW_TOO_LARGE: 'This document is too large to be shown here. It opens in another app, without leaving the device.',
	DOCUMENT_OPEN_FAILED: "Could not open this document.",
	ACCOUNT: "Account",
	COPY: "Copy",
	COPIED_TO_CLIPBOARD: "Copied to clipboard",
	CAMPUS_NOT_SUPPORTED_TITLE: 'Campus not connected yet',
	CAMPUS_NOT_SUPPORTED: 'This campus is not connected to UKit yet. The most requested campuses come first.',
	CAMPUS_REQUEST_ACTION: 'Request my campus',
	CAMPUS_REQUEST_SHORT: 'Request',
	RECORD_READ_LABEL: 'Last updated',
	LOADING_STARTING: "Opening the portal…",
	LOADING_FINISHING: "Almost done…",
	CREDENTIALS_UNVERIFIED: "Could not verify your credentials. Please try again.",
};

export default en;
